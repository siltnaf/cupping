...
Time.update=0;
Time.count++;
if (Time.count>=250) Time.blink=!Time.blink;
if (Time.count>=1000) 
        {
            Time.sec++;
            Time.count=0;
...
this_key->level++;
if (this_key->level>(Max_key-1)) this_key->level=0;
if (this_key->level==0) this_key->on=0;else this_key->on=1;
}
